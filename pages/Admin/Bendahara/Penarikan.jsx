...
export default function Transaksi({ data }) {
  ...
  return (
    <Layout>
      ...
      />
      <Head>
        <title>Penarikan - Bank Sampah Banyuwangi</title>
      </Head>
      <div className='border-b border-gray-400 pb-2 flex justify-between'>
        <h1 className='text-4xl'>Penarikan</h1>
        <div className='float-right flex space-x-2'>
          <Link href='/Client/Bendahara'>
            ...
            >
              ...
              <span className='align-middle'>Penarikan</span>
            </a>
          </Link>
        </div>
      </div>
...
export async function getServerSideProps(context) {
  ...
  const fetch1 = await fetch(
    `${process.env.NEXT_PUBLIC_API_HOST}/api/bankTransaction?limit=${limit}&page=${page}&transactionType=KREDIT&${queryString}`
  );
